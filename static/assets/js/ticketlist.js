document.addEventListener("DOMContentLoaded", function() {
  ...
  // Function to read a value from the 'fscs' cookie
  function readFromCookie(key) {
    let cookieValue = document.cookie.split('; ').find(row => row.startsWith('fscs='));
    if (cookieValue) {
      try {
        let data = JSON.parse(cookieValue.split('=')[1]); // Split by '=' and parse the JSON
  ...
  // Function to toggle the 'd-none' class based on cookie values
  function toggleElementVisibility() {
    // Check if the 'fscs' cookie exists
    if (doesCookieExist('fscs')) {
      // Read values for 'ticketStatus' and 'ticketProgressBar' from the cookie
      const ticketStats = readFromCookie('ticketStats');
      const ticketProgressBar = readFromCookie('ticketProgressBar');

      // Find DOM elements
      const ticketStatsElement = document.getElementById('ticketStats');
      const ticketProgressBarElement = document.getElementById('ticketProgressBar');

      // Toggle 'd-none' class based on the cookie values
      if (ticketStatsElement) {
        ticketStats ? ticketStatsElement.classList.remove('d-none') : ticketStatsElement.classList.add('d-none');
      }
      if (ticketProgressBarElement) {
        ticketProgressBar ? ticketProgressBarElement.classList.remove('d-none') : ticketProgressBarElement.classList.add('d-none');
      }
    }
  }
